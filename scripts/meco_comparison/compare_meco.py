...
def compare_to_meco(language, meco_webcam_dir):
    ...
    df_stats = pd.DataFrame(columns=['meco_TRT_mean', 'meco_nfix_mean', 'web_TRT_mean', 'web_nfix_mean'])
        ...
        for i, row in df.iterrows():
            try:
                if 'officialidentification' in row.word_id:
                    ...
                    df = df.sort_index().reset_index(drop=True)
                elif 'Kirmizi—Listesi"ni' in row.word_id:
                    df.loc[i] = [row.relFix, row.TRT, row.countFix, 'Kirmizi—', 'Kirmizi—', row['text_id'], row['sentence_id'], len('Kirmizi')]
                    df.loc[i + 0.5] = [np.nan, np.nan, np.nan, 'Listesi"ni', 'Listesi"ni', row['text_id'], row['sentence_id'], len('Listesi"ni')]
                    df = df.sort_index().reset_index(drop=True)
                elif 'galisan' in row.word_id:
                    df.loc[i + 0.5] = [np.nan, np.nan, np.nan, 'uluslararası', 'uluslararası', row['text_id'], row['sentence_id'], len('uluslararası')]
                    df = df.sort_index().reset_index(drop=True)
                elif row.word_id == '—' or row.word_id == '=':
                    df = df.drop([i])
            except TypeError:
                import ipdb;ipdb.set_trace()
    ...
    print(df_stats.sort_index())

    for (id, sen_web), (_, sen_meco) in zip(df.groupby('sentence_id'), df_meco.groupby('sentence_id')):
