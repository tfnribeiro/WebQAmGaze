...
class Study:
    ...
    def print_all_acc(self):
        ...
        for exp in self.experiment_list:
            ...
            if exp.features.avg_roi is None:
                utils.log_error(
                    "Skipped experiment due to error in validation step.", utils.Error.INFO, exp.worker_id)
        ...
        print(
            f"Experiment ROI/SampleRate avg: {total_experiment_roi.mean():.2f} / {total_experiment_sample_rate.mean():.2f}")
        print(
            f"Experiment ROI/SampleRate std: {total_experiment_roi.std():.2f} / {total_experiment_sample_rate.std():.2f}")
        print(
            f"Experiment ROI/SampleRate MAX: {total_experiment_roi.max():.2f} / {total_experiment_sample_rate.max():.2f}")
        print(
            f"Experiment ROI/SampleRate MIN: {total_experiment_roi.min():.2f} / {total_experiment_sample_rate.min():.2f}")
        print()
    ...
    def load_data_from_folder(self, path_to_data, is_psiturk: bool, path_to_setdata="",
        ...
        def get_config_file(data_dir):
            ...
            return False
        if path_to_setdata == "":
            path_to_setdata = os.path.join(
                __location__, "experiment_data", self.study_name)
            utils.log_error(
                f"No path to set given, defaulting to: '{path_to_setdata}'", utils.Error.INFO)
        utils.log_error(
            f"Loading data from: '{path_to_setdata}', is psiturk: '{is_psiturk}'", utils.Error.INFO)
        path_with_config = path_to_data
        target_data_path = os.path.join(
            path_to_setdata, "webgazer-sample-data.csv")
        self.get_study_targets(target_data_path)
        utils.log_error(
            f"Loading Target data from: '{target_data_path}', is psiturk: '{is_psiturk}'", utils.Error.INFO)
        if not os.path.isdir(path_with_config):
            # This is the case of psiturk
            path_with_config = os.sep.join(
                path_to_data.split(os.path.sep)[:-1])
        path_to_config = get_config_file(path_with_config)
        if path_to_config != False:
            self.load_config(path_to_config)

        if is_psiturk:
            experiment_loaded = self.load_psiturk(
                path_to_data, set_name_config=self.study_config.data["set_name"])
        else:
        ...
        if align_data:
            self.align_loaded_data(show_first_part_data, show_answers,
                                   export_target_dataframes, export_feature_dataframes)
    ...
    def align_loaded_data(self, show_first_part_data=False, show_answers=False, export_target_dataframes=False,
        ...
        for features in self.get_feature_vector_list():
            print(f"WorkerID: {features['worker_id']}, Type: {features['participant_type']}, Set: {features['set_name']} || Fixation Error | Target Error: ",
                  features['fixation_error'], features['target_error'])
            if export_feature_dataframes:
                features.to_json(os.path.join(
                    __location__, "pre_processed_data", f"{features.worker_id}_{features.set_name}.json"))
        if export_feature_dataframes:
            utils.log_error("All data saved in: " +
                            os.path.join("pre_processed_data"), utils.Error.INFO)
        fixation_error_ids = []
                ...
                if (not worker.features_series['fixation_error']
                    ...
                    try:
                        ...
                        for trial_name in worker.features_series['set_trials']:
                            duration_dataframe = worker.get_duration_fixation_on_word(
                                trial_name)
                            worker_target_df = pd.concat(
                                (worker_target_df, duration_dataframe))
                        worker_target_df.to_csv(os.path.join(__location__, "pre_processed_data", "fixation_data_per_part",
                                                f"{worker.worker_id}_{experiment_name}_fix_dict.csv"), index=False)
                    except Exception as e:
                        print(e)
                        fixation_error_ids.append(worker.worker_id)
            utils.log_error("All fixation data saved in: " + os.path.join(__location__,
                            "pre_processed_data", "fixation_data_per_part"), utils.Error.INFO)
        if len(fixation_error_ids) > 0:
            utils.log_error(
                "Participants had fixation errors when generating IDs", utils.Error.ERROR)
            utils.log_error(
                f"Total errors: {len(fixation_error_ids)}", utils.Error.ERROR)
    ...
    def load_cognition(self, folderpath, verbose=0):
        ...
        for file in os.listdir(folderpath):
            if "webgazer-sample-data.csv" in file:
                continue
            if "word_boundaries.csv" in file:
                continue
            if "trialdata.csv" in file:
                continue
            if ".csv" in file:
                print("Loading file: ", file)
                worker_id = "p" + \
                    file.replace(".csv", "")+"-"+folderpath.split(os.sep)[-1]
                counter_of_unique_ids += 1
                loaded_experiment_list.append(experiment(is_psiturk=False, filepath=os.path.join(
                    folderpath, file), img_path=folderpath, worker_id=worker_id))
    ...
    def load_psiturk(self, filepath, set_name_config=None, verbose=0):
        ...
        loaded_experiment_list = []
        set_name = filepath.split(
            os.sep)[-2] if set_name_config is None else set_name_config
        trial_data = pd.read_csv(
            filepath, names=['id', 'status', 'trial_id', 'data'])
        trial_ids_to_keep = []
        counter_of_unique_ids = 0
        for trial in trial_data.id.unique():
            ...
            if not_in_set_flag:
                if verbose > 1:
                    utils.log_error(
                        f"Assignment ID: {trial} not in this data_set. Value was ({trial_set_name}, set is: {set_name}.", utils.Error.ERROR, worker_id)
                continue
            ...
            if "trial_name" not in trial_df.columns:
                ...
                continue
            utils.log_error(
                f"Experiment loaded! Assignment ID: {asssingment_id}", utils.Error.INFO, worker_id)
            # print(f"Loaded data, ID: {trial}")
            # print("UNIQUE COUNT: ", len(trial_df['trial_name'].unique()))
            if len(trial_df['trial_name'].unique()) < 26:
                utils.log_error(
                    "Experiment has less trials than expected. The experiment was not terminated/missing data.", utils.Error.INFO, worker_id)
                # print(f"ERROR, Worker ID: {worker_id}, didn't seem to finish the experiment.")
                continue
            loaded_experiment_list.append(experiment(is_psiturk=True, img_path=os.path.join(
                *img_path), dataframe=trial_df, worker_id=worker_id, assignment_id=assignment_id))
        ...
        if len(trial_ids_to_keep) > 0:
            trial_data.loc[trial_ids_to_keep].to_csv(
                filepath, header=False, index=False)
            if verbose > 1:
                utils.log_error(
                    f"Total number of unique experiments: {counter_of_unique_ids}, writing to '{filepath}' to keep only the relevant data.", utils.Error.INFO)
        else:
            utils.log_error(
                f"No experiment was loaded, make sure the set_name matches the data you are trying to load.", utils.Error.ERROR)
        return loaded_experiment_list
    ...
    def set_config_experiment(self, temporary_list):
        ...
        utils.log_error(
            "Updated all the experiments according to configuration.", utils.Error.INFO)
    ...
    def update_targets_with_worker(self, worker_id_with_target, worker_id_with_error):
        ...
        if worker_id_correct is None or worker_id_error is None:
            utils.log_error(
                f"Reference Experiment ID: {worker_id_with_target} or Error Experiment ID: {worker_id_with_error} is missing.", utils.Error.ERROR)
            return
        if (worker_id_correct.features.screen_size[0] == worker_id_error.features.screen_size[0] and
            ...
            worker_id_error.webgazer_targets = copy.deepcopy(
                worker_id_correct.webgazer_targets)
            utils.log_error(
                f"WorkerID ({worker_id_with_error}) Webgazer tagets updated successfuly with WorkerID ({worker_id_with_target}) targets.", error_level=utils.Error.INFO)
            worker_id_error.recalculate_features()
        else:
            utils.log_error(
                f"Targets were not updated as experiment with error and reference screen sizes are not the same. Error ID: {worker_id_error.worker_id} | Reference ID: {worker_id_correct.worker_id}", utils.Error.ERROR)
    ...
    def print_all_total_time(self):
        ...
        for exp in self.experiment_list:
            ...
            print(
                f"WorkerID: {exp.worker_id} | Total Time (min): {total_time_to_min}")
    ...
    def get_study_answers(self):
        ...
        for exp in self.experiment_list:
            ...
            for key, pair in exp.trial_answers.items():
                ...
                print(
                    f"Total fixations on target: {exp.features.counts_per_target[key]}/{exp.features.counts_per_target[key][key]}")
                print()
    ...
    def print_corrected_answers(self):
        ...
        for worker, answers in self.corret_answers.items():
            ...
            print(
                f"## Showing results for workerID: {worker}, total time: {worker_exp_total_time:.2f} ##")
            print()
            ...
            for key, (score, fix_ratio) in answers.items():
                if score == 1:
                    print(
                        f"{key}: CORRECT (1), with a fixation ratio of {fix_ratio:.2f}")
                    count_correct += 1
                else:
                    print(
                        f"{key}: WRONG (0), with a fixation ratio of {fix_ratio:.2f}")

            print()
            print(
                f"WorkerID: {worker} has {count_correct}/{len(answers)} correct answers.")
            if worker_exp_total_time < 10:
                print()
                print(
                    f"###!!! CHECK EVENTS FOR USER: {worker}, experiment took too little time. !!!###")
                print()
            if round(count_correct/len(answers), 3) >= 0.50:
                counter_approved += 1
                if round(count_correct/len(answers), 3) >= 0.75:
                    ...
                    print(
                        f"RUN PsiTurk: 'worker bonus --amount 2.00 {assignment_id}'")
                    print(
                        f"Message: 'Well done, you got at least 75% of the questions correct!'")
                elif round(count_correct/len(answers), 3) >= 0.60:
                    counter_bonus_one += 1
                    ...
                    print(f"RUN PsiTurk: 'worker approve {assignment_id}'")
                    print(
                        f"RUN PsiTurk: 'worker bonus --amount 1.00 {assignment_id}'")
                    ...
                    print(
                        f"Message: \"Well done, you got at least 60% of the questions correct!\"")
                else:
    ...
    def correct_study_answers(self, verbose=False):
        ...
        path_dict_accepted_answer = os.path.join(
            __location__, "experiment_data", self.study_name, "accepted_answers.json")
        path_dict_unaccepted_answer = os.path.join(
            __location__, "experiment_data", self.study_name, "unaccepted_answers.json")
        ...
        for exp in self.experiment_list:
            for key, pair in exp.trial_answers.items():
                ...
                try:
                    rt_per_trial[paragraph_target].append(
                        exp.data[exp.data.trial_name == paragraph_target].rt.values[0])
                except:
                    utils.log_error("Worker completed a task twice!",
                                    utils.Error.WARNING, exp.worker_id, paragraph_target)
        ...
        for exp in self.experiment_list:
            ...
            for i, (key, pair) in enumerate(exp.trial_answers.items()):
                ...
                try:
                    time_taken = exp.data[exp.data.trial_name ==
                                          paragraph_target].rt.values[0]
                except:
                    print(exp.data[exp.data.trial_name == paragraph_target])
                    utils.log_error("Worker completed a task twice!",
                                    utils.Error.WARNING, exp.worker_id, paragraph_target)
                    # print(f"## WARNING!! WorkerID: {exp.worker_id}, completed task [{paragraph_target}] twice!!")
                avg_paragraph_time = np.array(
                    rt_per_trial[paragraph_target]).mean()
                std_paragraph_time = np.array(
                    rt_per_trial[paragraph_target]).std()
                if verbose:
                    print(
                        f"Trial: {key} | A: {pair[0]} || C: {correct_answer}")
                    print(
                        f"User took: {time_taken}ms, avg time taken in trial is: {avg_paragraph_time:.2f}, std={std_paragraph_time:.2f}")
                if time_taken < avg_paragraph_time-(avg_paragraph_time*.1):
                    if verbose:
                        print("----THIS USER TOOK LESS THAN AVERAGE ----")
                        print("Data for RT for this task: ",
                              rt_per_trial[paragraph_target])
                ...
                try:
                    ...
                    if verbose:
                        print(
                            f"Total fixations on target: {fixations_on_target}/{total_fixations}")
                except Exception as e:
                    utils.log_error(
                        f"Fixations contain errors. Exception: {e}", utils.Error.ERROR, exp.worker_id, paragraph_target)
                    utils.log_error(
                        f"Key that was used: {exp_key}", utils.Error.ERROR, exp.worker_id, paragraph_target)
                    utils.log_error(
                        f"Dictionary where error was seen: {exp.features.counts_per_target[paragraph_target]}", utils.Error.INFO)
                    error_load_fixations = True

                if answer in dict_unaccepted_answer[key]:
                    if verbose:
                        print(
                            f"WorkerID ({exp.worker_id}), Trial {key} is wrong!")
                    correct = "n"
                elif answer in dict_accepted_answer[key]:
                    if verbose:
                        print(
                            f"WorkerID ({exp.worker_id}), Trial {key} is correct!")
                    correct = "y"
                else:
                ...
                if "y" in correct:
                    ...
                    if error_load_fixations or total_fixations == 0:
                        ...
                        self.corret_answers[exp.worker_id][key] = (
                            1, fixations_on_target/total_fixations)
                    ...
                    if error_load_fixations or total_fixations == 0:
                        ...
                        self.corret_answers[exp.worker_id][key] = (
                            0, fixations_on_target/total_fixations)
                        exp.features_series[f"question_{i}_target_to_fixation_ratio"] = fixations_on_target/total_fixations
    ...
    def get_study_settings(self, filepath):
        experiment_dev = experiment(
            filepath=filepath, is_dev=True, is_psiturk=False)
        return experiment_dev.data

    def export_set_texts(self, filepath, export_dataframe=True):
        experiment_dev = experiment(
            filepath=filepath, is_dev=True, is_psiturk=False)
        dataframe_with_settings = experiment_dev.data[[
            'stimulus', 'trial_name', 'correct_answer', 'question_text']].copy()
        dataframe_with_settings = dataframe_with_settings.dropna(
            subset="trial_name")
        ...
        for i, row in dataframe_with_settings.iterrows():
            ...
            if pd.isnull(string_to_update):
                new_stimulus.append(html_pattern.sub(
                    "", row["question_text"]).strip())
            else:
                new_stimulus.append(html_pattern.sub(
                    "", string_to_update).strip())
        dataframe_with_settings.stimulus = new_stimulus
        dataframe_with_settings["lang"] = filepath.split(
            os.sep)[-2][len("mturk_"):len("mturk_")+2]
        ...
        if export_dataframe:
            path_to_save = os.path.join(
                "experiment_data", "set_texts", f"text_settings_{filepath.split(os.sep)[-2]}.csv")
            dataframe_with_settings[[
                'stimulus', 'trial_name', 'task_type', 'correct_answer', 'lang']].to_csv(path_to_save)
        return dataframe_with_settings

    def create_word_target_file(self, experiment_path):
        df = pd.DataFrame(
            columns=['text', 'word', 'counter', 'x', 'y', 'width', 'height'])
        ii = 0
        for file in os.listdir(experiment_path):
            if file.endswith('.png'):
                words = []
                img = cv2.imread(os.path.join(
                    __location__, experiment_path, file))
                ...
                for i in range(n_boxes):
                    if (d['text'][i] != ""):
                        ...
                        df.loc[ii] = [file.split(".")[0], d['text'][i], Counter(words)[d['text'][i].lower(
                        ).replace(".", "").replace(',', '').replace(';', '')], x, y, w, h]
                        words.append(d['text'][i].lower().replace(
                            ".", "").replace(',', '').replace(';', ''))
                        ii += 1
        path_to_save = os.path.join(
            __location__, experiment_path, "word_boundaries.csv")
        df.to_csv(path_to_save)
        utils.log_error(
            f"Bounding boxes areas saved to: {path_to_save}", utils.Error.INFO)
    ...
    def get_study_targets(self, filepath, export_dataframe=True, load_word_boundaries=True):
        ...
        if load_word_boundaries:
            ...
            path_to_word_boundaries_csv = os.path.join(
                path_to_set_dir, "word_boundaries.csv")
            if not os.path.exists(path_to_word_boundaries_csv):
                utils.log_error(
                    f"Creating Word Boundaries for words in dir: {path_to_set_dir}", utils.Error.INFO)
                self.create_word_target_file(path_to_set_dir)
            else:
                utils.log_error(
                    f"Loaded file from: {path_to_word_boundaries_csv}", utils.Error.INFO)
            word_boundaries_df = pd.read_csv(path_to_word_boundaries_csv)
            word_boundaries_df["text"] = word_boundaries_df["text"].str.replace(
                "_1280_720", "")
            for k, target_list in self.target_dict.items():
                ...
                for row in targets_to_add.iterrows():
                    ...
                    if len(dictionary_line_ys.get(current_line, [])) == 0:
                        ...
                        dictionary_line_height[current_line] = dictionary_line_height[current_line] + [
                            t_height]
                ...
                for k_ys, v in dictionary_line_ys.items():
                    if "meco" in k:
                        dictionary_line_ys[k_ys] = np.array(
                            v).mean() - MECO_Y_ADJUSTMENT/2
                    else:
                        dictionary_line_ys[k_ys] = np.array(
                            v).mean() - XQUAD_Y_ADJUSTMENT/2

                for k_heights, v in dictionary_line_height.items():
                    # It seems height is over estimated so I have made remove a pixel
                    if "meco" in k:
                        dictionary_line_height[k_heights] = np.array(
                            v).mean() + MECO_Y_ADJUSTMENT
                    else:
                        dictionary_line_height[k_heights] = np.array(
                            v).mean() + XQUAD_Y_ADJUSTMENT

                for target in target_list:
                    if "#" not in target.name:
                        if "meco" in k:
                            target.y -= MECO_Y_ADJUSTMENT/2 + WEBGAZER_TARGET_ADJUSTMENT/2
                            target.height += MECO_Y_ADJUSTMENT + \
                                (WEBGAZER_TARGET_ADJUSTMENT +
                                 HTML_BOUNDING_ADJUSTMENT)
                        else:
                            target.y -= XQUAD_Y_ADJUSTMENT/2 + WEBGAZER_TARGET_ADJUSTMENT/2
                            target.height += XQUAD_Y_ADJUSTMENT + \
                                (WEBGAZER_TARGET_ADJUSTMENT +
                                 HTML_BOUNDING_ADJUSTMENT)
                ...
                for i, row in enumerate(targets_to_add.iterrows()):
                    ...
                    new_target = WebgazeTarget(
                        t_name, t_x, t_y, t_width, t_height, -1, -1, -1, -1, is_word=True)
        ...
        if export_dataframe:
            column_names = ["text_name", "target_name", "x", "y", "width",
                            "height", "top", "right", "bottom", "left", "is_word"]
                ...
                for value in values:
                    if key in value.name:
                        ...
                        new_series = pd.Series(
                            [key] + [value.name] + list(value.to_numpy()), index=column_names)
                    else:
                        # The target was a generic #webgazer_target
                        new_series = pd.Series(
                            [key] + [key+"_"+value.name] + list(value.to_numpy()), index=column_names)
                    data_frame_target_data.append(new_series)
            pd.DataFrame(data_frame_target_data).to_csv(os.path.join(
                __location__, "target_dataframes", f"target_dataframe_{filepath.split(os.sep)[-2]}.csv"))
    ...
    def update_all_experiments_targets(self):
        ...
        """
        utils.log_error(
            "Updating all experiment targets & Recalculating Features...", utils.Error.INFO)
        # print("Updating all experiment targets & Recalculating Features...")
        experiment_loaded_correctly = np.ones(
            len(self.experiment_list), dtype=bool)
        ...
        if total_errors > 0:
            ...
            workers_fixed_c = 0
            workers_with_no_targets = [i for i, v in enumerate(
                experiment_loaded_correctly) if not v]
            utils.log_error(
                "Attempting to correct fixations with another worker...", utils.Error.INFO)
            ...
            for worker_error_i in workers_with_no_targets:
                ...
                for w_i, exp in enumerate(self.experiment_list):
                    ...
                    if (w_i not in workers_with_no_targets and
                        ...
                        self.update_targets_with_worker(
                            exp.worker_id, worker_error_exp.worker_id)
                        experiment_loaded_correctly[worker_error_i] = self.update_exp_targets(
                            worker_error_exp)
                        workers_fixed_c += 1
                        # Ensure the value was updated.
                        assert worker_error_exp.features_series['target_error'] == False
                        assert experiment_loaded_correctly[
                            worker_error_i], f"ERROR: Updating the user failed. Please check WorkerID: {worker_error_exp.worker_id}"
                        break
                if not experiment_loaded_correctly[worker_error_i]:
                    utils.log_error(
                        f"No workers found with same screen size as {worker_error_exp.worker_id}, ({worker_error_screen_size}).", utils.Error.INFO)
            ...
            if np.logical_not(experiment_loaded_correctly).any():
                workers_with_no_targets = [self.experiment_list[i].worker_id for i, v in enumerate(
                    experiment_loaded_correctly) if not v]
                for i, v in enumerate(experiment_loaded_correctly):
                    if not v:
                        self.experiment_list[i].features_series['target_error'] = True
                utils.log_error(
                    f"Remaining workers with no targets: {workers_with_no_targets}", utils.Error.INFO)
                # print(f"WARNING: {workers_fixed_c}/{total_errors} Workers with target errors after attempting to fix: ", workers_with_no_targets)
            utils.log_error(
                f"Number of Worker with no targets fixed: {workers_fixed_c}/{total_errors}.", utils.Error.INFO)
        ...
        utils.log_error(
            "STEP DONE! 'Updating all experiment targets & Recalculating Features...'", utils.Error.INFO)
    ...
    def plot_experiment_data(self, plot_type, filtered_data=False):
        ...
        assert plot_type in [
            "x_axis", "y_axis", "scatter"], f"Invalid plot type, valid options: {valid_options}"
        ...
        for trial in wegbazer_experiment_list:
            for i_p, person_data in enumerate(self.experiment_list):
                ...
                if plot_type == "x_axis" and len(data_to_plot) > 0:
                    ...
                    plt.scatter(data_to_plot[:, 0],
                                data_to_plot[:, 1], label=i_p)
...
if __name__ == '__main__':
    ...
    if len(args) == 0:
        ...
        mode = args['mode'][0]
        psiturk_default_data_path = os.path.join(
            __location__, "experiment_data", study_name, "trialdata.csv")
        csv_default_data_path = os.path.join(
            __location__, "experiment_data", study_name)
        ...
        if mode == "p":
            ...
            try:
                ...
                # The file has multiple experiments for the same set, load the psiturk
                utils.log_error(
                    "Failed to load file in default directory, attempting in sub-folders...", utils.Error.WARNING)
                default_data_path = os.path.join(
                    __location__, "experiment_data", study_name)
                for directory_w_data in os.listdir(os.path.join(default_data_path)):
                    dir_path = os.path.join(
                        default_data_path, directory_w_data)
                    if not os.path.isdir(dir_path):
                        continue
                    if os.path.exists(os.path.join(dir_path, "trialdata.csv")):
                        set_data.load_data_from_folder(os.path.join(dir_path, "trialdata.csv"), True, os.path.join(__location__, "experiment_data", study_name,),
                                                       align_data=False)
        elif mode == "c":
            ...
            test_recreate_set = [
                name for name in set_data[set_data.trial_name.notna()].trial_name]
            xquad_string = "let reconstruct_trial_xquad = ["
            for i in range(0, len(test_recreate_set), 2):
                if i == 0:
                    print(
                        f"let reconstruct_trial_meco = [[{test_recreate_set[i]}, [{test_recreate_set[i+1]}]]];")
                else:
            ...
            utils.log_error(
                "## BETA. This functionality needs to be updated for the new data storing format.", utils.Error.WARNING)
            ...
            for study_name in os.listdir("experiment_data"):
                ...
                if not os.path.exists(os.path.join(__location__, "experiment_data", study_name, "accepted_answers.json")):
                    ...
                    utils.log_error(
                        f"Set {study_name} has not been corrected yet, skipping...", utils.Error.INFO)
                    continue
                ...
                if export_text_bool:
                    test_study.export_set_texts(os.path.join(
                        __location__, "experiment_data", study_name, "webgazer-sample-data.csv"))
                ...
                for worker in test_study.experiment_list:
                    ...
                    if (not worker.features_series['fixation_error']
                        ...
                        for trial_name in worker.features_series['set_trials']:
                            ...
                            worker_target_df = pd.concat(
                                (worker_target_df, duration_dataframe))
                        worker_target_df.to_csv(os.path.join(
                            __location__, "pre_processed_data", "fixation_data_per_part", f"{worker.worker_id}_{study_name}_fix_dict.csv"), index=False)
            ...
            if filter_approved:
                ...
                all_features_filtered = all_features_filtered[
                    all_features_filtered["fixation_error"] == False]
                ...
                all_features_filtered = all_features_filtered[
                    all_features_filtered["webgazer_sample_rate"] > 10]
                ...
                for row_i, row in all_features_filtered.iterrows():
                    row.to_json(os.path.join(
                        __location__, "pre_processed_data", f"{row.worker_id}_{row.set_name}.json"))
                utils.log_error(
                    f"Filtered data saved to: {save_path}", utils.Error.INFO)
            else:
                for row_i, row in all_features_export.iterrows():
                    row.to_json(os.path.join(
                        __location__, "pre_processed_data", f"{row.worker_id}_{row.set_name}.json"))
                utils.log_error(
                    f"All data saved to: {save_path}", utils.Error.INFO)

            utils.log_error(
                f"Shape of dataframe         : {all_features_export.shape}", utils.Error.INFO)
            utils.log_error(
                f"Shape of filtered dataframe: {all_features_filtered.shape}", utils.Error.INFO)
        else:
            parser.print_help()
